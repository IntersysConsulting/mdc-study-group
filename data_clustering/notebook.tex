...
records in different clusters are quite disctinct.
...
will review the standard algorithm.
...
\(\gamma_1^{(0)}\), \(\gamma_{n - 1}^{(0)}\) as follows:
\begin{equation}
...
\(\gamma_{n - 1}^{(0)}\), the \emph{k-means} algorithm updates the
cluster centers as follows: \begin{equation}
\label{center-update}
...
memberships based on Equation \ref{membership-update} and
\ref{center-update}.
